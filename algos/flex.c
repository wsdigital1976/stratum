...
void flex_hash(const char* input, char* output, uint32_t len) {
	...
	sph_sha3_512_context ctx_keccak;
	...
	void *in = (void*) input;
	int size = 80;
    sph_sha3_512_init(&ctx_keccak);
    sph_sha3_512(&ctx_keccak, in, size);
    sph_sha3_512_close(&ctx_keccak, hash);
	...
	for (i = 0; i < 18; i++)
	{
		...
		{
		 case CNDark:
			cryptonightdark_hash_flex(in, hash, size, 1);
			break;
		 case CNDarklite:
			cryptonightdarklite_hash_flex(in, hash, size, 1);
			break;
		 case CNFast:
			cryptonightfast_hash_flex(in, hash, size, 1);
			break;
		 case CNLite:
			cryptonightlite_hash_flex(in, hash, size, 1);
			break;
		 case CNTurtle:
			cryptonightturtle_hash_flex(in, hash, size, 1);
			break;
		 case CNTurtlelite:
			cryptonightturtlelite_hash_flex(in, hash, size, 1);
			break;
		}
		//selection core algo
		switch (algo) {
...
/* 		if(cnSelection >= 0) {
    ...
    sph_sha3_256_init(&ctx_keccak);
    sph_sha3_256(&ctx_keccak, in, size);
    sph_sha3_256_close(&ctx_keccak, hash);
