...
static void do_turtle_skein_hash_flex(const void* input, size_t len, char* output) {
...
static void (* const extra_hashes_flex[3])(const void *, size_t, char *) = {
    do_turtle_blake_hash, do_turtle_groestl_hash, do_turtle_skein_hash_flex
};
...
void cryptonightturtle_hash_flex(const char* input, char* output, uint32_t len, int variant) {
#if defined(_MSC_VER)
    struct cryptonightturtle_ctx *ctx = _malloca(sizeof(struct cryptonightturtle_ctx));
#else
    struct cryptonightturtle_ctx *ctx = alloca(sizeof(struct cryptonightturtle_ctx));
