...
static void do_skein_hash_flex(const void* input, size_t len, char* output) {
...
static void (* const extra_hashes_flex[3])(const void *, size_t, char *) = {
    do_blake_hash, do_groestl_hash, do_skein_hash_flex
};
...
void cryptonight_hash_flex(const char* input, char* output, uint32_t len, int variant) {
#if defined(_MSC_VER)
    struct cryptonight_ctx *ctx = _malloca(sizeof(struct cryptonight_ctx));
#else
    struct cryptonight_ctx *ctx = alloca(sizeof(struct cryptonight_ctx));
