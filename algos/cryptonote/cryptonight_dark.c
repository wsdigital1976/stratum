...
static void do_dark_skein_hash_flex(const void* input, size_t len, char* output) {
...
static void (* const extra_hashes_flex[3])(const void *, size_t, char *) = {
    do_dark_blake_hash, do_dark_groestl_hash, do_dark_skein_hash_flex
};
...
void cryptonightdark_hash_flex(const char* input, char* output, uint32_t len, int variant) {
#if defined(_MSC_VER)
    struct cryptonightdark_ctx *ctx = _malloca(sizeof(struct cryptonightdark_ctx));
#else
    struct cryptonightdark_ctx *ctx = alloca(sizeof(struct cryptonightdark_ctx));
