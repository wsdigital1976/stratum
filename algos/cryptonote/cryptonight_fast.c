...
static void do_fast_skein_hash_flex(const void* input, size_t len, char* output) {
...
static void (* const extra_hashes_flex[3])(const void *, size_t, char *) = {
    do_fast_blake_hash, do_fast_groestl_hash, do_fast_skein_hash_flex
};
...
void cryptonightfast_hash_flex(const char* input, char* output, uint32_t len, int variant) {
    struct cryptonightfast_ctx *ctx = malloc(sizeof(struct cryptonightfast_ctx));
    hash_process(&ctx->state.hs, (const uint8_t*) input, len);
    memcpy(ctx->text, ctx->state.init, INIT_SIZE_BYTE);
    memcpy(ctx->aes_key, ctx->state.hs.b, AES_KEY_SIZE);
